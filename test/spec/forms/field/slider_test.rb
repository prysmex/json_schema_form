...
class SliderTest < Minitest::Test
  ...
  def test_enum_spacing
    ...
    enums.each do |enum|
      ...
      # no errors
      assert_empty instance.errors(if: ->(_obj, key) { key == :enum_interval })

      # force errors
      instance[:enum].map! { |v| v + rand(100) }

      refute_empty instance.errors(if: ->(_obj, key) { key == :enum_interval })
    end
  end
  ...
  def test_max_precision
    ...
    # no error
    assert_empty instance.errors(if: ->(_obj, key) { key == :enum_precision })

    instance[:enum] = [move_decimail_point(1, JSF::Forms::Field::Slider::MAX_PRECISION + 1)]

    refute_empty instance.errors(if: ->(_obj, key) { key == :enum_precision })
  end
