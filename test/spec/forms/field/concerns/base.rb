...
module BaseFieldTests
  ...
  def test_id_regex
    ...
    refute_nil tested_klass.new({'$id': '/$defs/hello'}).errors[:$id]
    refute_nil tested_klass.new({'$id': '#/$defs/hello'}).errors[:$id]
  end
  ...
  def test_verify_default
    ...
    refute_empty prop.errors(if: ->(_obj, key) { key == :verify_default })
  end
  ...
  def test_scored?
    instance = tested_klass.new(tested_klass_example)
