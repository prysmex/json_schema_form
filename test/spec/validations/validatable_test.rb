...
class ValidatableTest < Minitest::Test
  ...
  def test_conditional_errors
    ...
    # if
    refute_empty SampleSchema.new({}).errors(if: ->(_instance, key) { key == :id_presence })
    assert_empty SampleSchema.new({}).errors(if: ->(_instance, key) { key != :id_presence })

    # unless
    assert_empty SampleSchema.new({}).errors(unless: ->(_instance, key) { key == :id_presence })
    refute_empty SampleSchema.new({}).errors(unless: ->(_instance, key) { key != :id_presence })
  end
