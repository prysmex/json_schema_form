module JSF
  module Forms
    ...
    class Form < BaseHash
      ...
      ATTRIBUTE_TRANSFORM = ->(attribute, subschema, instance, init_options) {
        ...
        when JSF::Forms::Form
          ...
          when 'definitions'
            if subschema[:isResponseSet]
              return JSF::Forms::ResponseSet.new(subschema, init_options)
            elsif subschema[:type] == 'object' #replaced schemas
              return JSF::Forms::Form.new(subschema, init_options)
            elsif subschema.key?(:$ref) # shared definition
              return JSF::Schema.new(subschema, init_options.merge(preinit_proc: SUBSCHEMA_PROC))
            end
          when 'allOf'
            return JSF::Schema.new(subschema, init_options.merge(preinit_proc: SUBSCHEMA_PROC))
          when 'properties'
            if subschema.key?(:$ref)
              if subschema.dig(:displayProperties, :isSelect)
                return JSF::Forms::Field::Select.new(subschema, init_options)
              else
                return ::JSF::Forms::Field::Component.new(subschema, init_options)
              end
            end
